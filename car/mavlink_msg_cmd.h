...
#define MAVLINK_MSG_ID_CMD_LEN 1
#define MAVLINK_MSG_ID_CMD_MIN_LEN 1
#define MAVLINK_MSG_ID_13_LEN 1
#define MAVLINK_MSG_ID_13_MIN_LEN 1

#define MAVLINK_MSG_ID_CMD_CRC 196
#define MAVLINK_MSG_ID_13_CRC 196
...
#define MAVLINK_MESSAGE_INFO_CMD { \
    ...
    "CMD", \
    1, \
    {  { "cmd", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_cmd_t, cmd) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_CMD { \
...
{
    return mavlink_msg_cmd_pack(system_id, component_id, msg, cmd->cmd);
}
...
{
    return mavlink_msg_cmd_pack_chan(system_id, component_id, chan, msg, cmd->cmd);
}
...
static inline void mavlink_msg_cmd_send(mavlink_channel_t chan, uint8_t cmd)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_CMD_LEN];
    _mav_put_uint8_t(buf, 0, cmd);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_cmd_send(chan, cmd->cmd);
#else
 ...
 */
static inline void mavlink_msg_cmd_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t cmd)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
